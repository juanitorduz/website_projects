...
$$
\end{itemize}
{\bf Example:} The map $f(x) = \phi(x)^T b $ (with prior $b\sim N(0, \Sigma_p)$) defines a Gaussian process with $m(x)=0$ and $k(x, x') = \phi(x)^T\Sigma_p\phi(x)$. 
\end{frame}
...
\begin{center}
\begin{figure}
\includegraphics[scale=0.16]{images/prior_func_lin_reg.png} 
\includegraphics[scale=0.15]{images/posterior_func_lin_reg.png} 
\caption{Prior (left) and posterior (right) distribution for linear model.}
\end{figure}
\end{center}
\end{frame}
