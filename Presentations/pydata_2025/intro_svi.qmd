...
## What is Variational Inference? {.smaller}
...
- $\displaystyle {p(D) = \int p(D|\theta)p(\theta)d\theta}$: [**Hard to compute!**]{style="color:red;"}
...
![](intro_svi_files/static/images/logos/jax_logo.svg){fig-align="center" width="250"}
...
![](intro_svi_files/static/images/logos/flax_logo.svg){fig-align="center" width="120"}
...
::: footer
[https://docs.jax.dev/en/latest/](https://docs.jax.dev/en/latest/)
:::
...
- [**Maximizing the ELBO is equivalent to minimizing the KL divergence between our approximate posterior $q_\phi(\theta)$ and the true posterior $p(\theta|D)$ (up to a constant, with respect to $q_\phi(\theta)$).**]{style="color:red;"} **Why? Use the Bayes' rule to expand the KL divergence.**
