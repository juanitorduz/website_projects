...
$$
(- \nabla^2)^{s} f(x) = \int_{\Omega} \overbrace{\left[\sum_{j} \lambda_{j}^{s} \phi_{j}(x) \phi_{j}(x')\right]}^{l^s(x, x')} f(x') dx'.
$$
...
Finally, recall that the coefficient $a_j$ where defined by the polynomial expansion of the spectral density, so if we take $||\omega||^2 = \lambda_j$ then we arrive at the final expression for the approximation formula:
