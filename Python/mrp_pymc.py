...
#
# The idea is to split the question in two. First, we ask what each *type* of person answers: we fit a model that predicts the outcome from demographics and geography. Many cells of this cross-classification have five respondents or none, so we fit the model hierarchically and let small cells borrow strength from the rest. Second, we ask how many people of each type there are in the population. We take these counts from a census and average the model predictions with the counts as weights. The survey informs the first step and the census informs the second.
#
# In this notebook we state the problem and the estimand, we write the assumption behind MRP as a causal DAG, and we show that the poststratification step is the g-formula of causal inference. Then we implement the method in PyMC. We simulate the population and the biased survey with a generative PyMC model, we fit the MRP model with the experimental [`pymc.dims`](https://www.pymc.io/projects/docs/en/stable/learn/core_notebooks/dims_module.html) module, and we compute the poststratified estimates inside the model. At the end we break the identifying assumption on purpose and watch the method fail while every diagnostic stays clean. We assume the reader knows the basics of PyMC and of hierarchical models.
#
...
#
# We start with the abstract problem, because the method only makes sense once the target is clear.
#
...
# %% [markdown]
# **Failure mode 1: the answer drives the response.** People who would answer "yes" respond more often than people who would answer "no". The arrow $Y \to I$ creates a dependence between $Y$ and $I$ inside every cell. No poststratification frame can fix this, because the bias lives within the cells. This is non-ignorable non-response, and it needs a model of the response mechanism, not a bigger census table.
...
dag.node("Y", label="Y\n(answer)", color="#328c0680", style="filled")
dag.node("I", label="I = 1\n(responded)", shape="box", color="#fa7c1780", style="filled")
dag.node("U", label="U\n(unobserved, e.g. political interest)", color="lightgray", style="filled")
...
# %% [markdown]
# **Failure mode 2: an unmeasured common cause.** A variable $U$ that we did not measure, such as political interest, drives both the response and the answer. Inside a cell defined by $X$, the path $I \leftarrow U \to Y$ is open, so $Y \not\perp I \mid X$. The fix is to measure $U$ and to add it to the model *and* to the frame. This promotes $U$ into $X$ and brings us back to the first graph.
#
...
#
# The second step of MRP is not a survey trick. It is the same computation as standardization, or the g-formula, in causal inference. We make the comparison explicit.
#
# **The causal side.** Let $A \in \{0, 1\}$ be a treatment and let $Y^a$ be the potential outcome, the value of $Y$ a person would have if we set $A = a$. Let $X$ be the covariates that affect both the treatment and the outcome. The target is $\mathbb{E}[Y^a]$, the mean outcome if everyone received treatment $a$. We cannot read it off the data, because in observational data the treated and the untreated differ in $X$. Three assumptions identify the target: conditional exchangeability $Y^a \perp A \mid X$, positivity $0 < P(A = a \mid X = x) < 1$, and consistency, which says that the observed outcome equals $Y^a$ for the treatment actually received. Then
#
# $$\mathbb{E}[Y^a] = \sum_x \mathbb{E}[Y \mid A = a, X = x] \, P(X = x).$$
#
...
#
# **The survey side.** Now let $I$ be the response indicator. We only see respondents, so the quantity we can estimate is $\mathbb{E}[Y \mid X = x, I = 1]$. The target is $\mathbb{E}[Y]$ in the whole population. Under $Y \perp I \mid X$ and positivity $P(I = 1 \mid X = x) > 0$ we get
#
# $$\mathbb{E}[Y] = \sum_x \mathbb{E}[Y \mid X = x, I = 1] \, P(X = x), \qquad P(X = x_j) = \frac{N_j}{N}.$$
#
# This is $\theta^{\text{PS}}$ from Step 2, with $\theta_j$ as the conditional mean and the census shares as $P(X)$. It is also the recoverability formula of [Bareinboim, Tian and Pearl (2014)](https://ftp.cs.ucla.edu/pub/stat_ser/r425.pdf) for selection bias with external information on $P(X)$: the same expression, derived from the graph. The two DAGs below have the same shape.
...
# %% [markdown]
# In both graphs $X$ is the common cause we adjust for. The differences are the node we act on and one arrow. In the causal case we intervene on $A$ and ask for the outcome under $A = a$. In the survey case we "intervene" on $I$ and ask for the outcome if everyone had responded. And $I$ has no arrow into $Y$: responding does not change the answer, so the target is simply $\mathbb{E}[Y]$.
#
# | | g-formula (causal effect) | poststratification (MRP) |
# |---|---|---|
# | target | $\mathbb{E}[Y^a]$ | $\mathbb{E}[Y]$ in the population |
# | what the data can estimate | $\mathbb{E}[Y \mid A = a, X = x]$ | $\mathbb{E}[Y \mid X = x, I = 1]$ |
# | distribution we average over | $P(X = x)$, from the sample or from a target population | $N_j / N$, from the census |
# | what the adjustment removes | confounding | selection bias |
# | identifying independence | $Y^a \perp A \mid X$ | $Y \perp I \mid X$ |
# | positivity | $0 < P(A = a \mid X = x) < 1$ | $P(I = 1 \mid X = x_j) > 0$ for all cells with $N_j > 0$ |
...
# 3. We can change the population. If we replace $P(X)$ by the covariate distribution $P^*(X)$ of a different target population, we get the transport formula $\sum_x \mathbb{E}[Y \mid X = x, I = 1] \, P^*(X = x)$. Poststratifying with a new set of counts $N^*_j$ is the same thing, and we do it below without refitting. An experiment run on a non-representative group of participants needs both adjustments at once: $\sum_x \mathbb{E}[Y \mid A = a, X = x, I = 1] \, P^*(X = x)$.
...
def make_generative_model(response_ratio: float = 1.0) -> pm.Model:
    with pm.Model(coords=coords) as model:
        ...
        N = pmd.LogNormal("N", mu=np.log(50_000), sigma=0.7, dims=frame_dims)
        ...
        theta = pmd.Deterministic("theta", pmd.math.sigmoid(a0 + b_age + b_eth + b_state))
        pmd.Deterministic("theta_ps", (N * theta).sum(frame_dims) / N.sum(frame_dims))
        pmd.Deterministic("theta_ps_state", (N * theta).sum(("age", "eth")) / N.sum(("age", "eth")))
        ...
        p_sel = pmd.Deterministic("p_sel", (propensity * N) / (propensity * N).sum(frame_dims))
...
print(f"cells: {N_cell.size}, population: {N_cell.sum():,}, respondents: {n_cell.sum():,}")
...
frame = pl.DataFrame(
    ...
    N=pl.Series(N_cell.ravel()),
    n=pl.Series(n_cell.ravel()),
    y=pl.Series(y_cell.ravel()),
    theta=pl.Series(theta_true.ravel()),
)
...
for ax, factor in zip(axes, ["age", "eth"], strict=True):
    ...
    ax.set(xlabel=factor, ylabel="share")

axes[0].legend()
fig.suptitle("The sample does not look like the population", fontsize=18, fontweight="bold");
...
# %%
def make_mrp_model(N: np.ndarray, v: np.ndarray, n: np.ndarray, y: np.ndarray) -> pm.Model:
    with pm.Model(coords=coords) as model:
        # data: census counts, state-level predictor and respondents per cell
        N_data = pmd.Data("N", N.astype(float), dims=frame_dims)
        ...
        pmd.Deterministic("theta_ps", (N_data * theta).sum(frame_dims) / N_data.sum(frame_dims))
        pmd.Deterministic("theta_ps_state", (N_data * theta).sum(("age", "eth")) / N_data.sum(("age", "eth")))

        # likelihood: the dims module has no Binomial yet, so we use regular tensors
        pm.Binomial("y", n=n_data.values, logit_p=eta.values, observed=y, dims=frame_dims)
...
# %% [markdown]
# ### Does the Deterministic Change Inference?
#
# No. Deterministic nodes are evaluated after each accepted draw and are not part of the gradient computation, so NUTS behaves the same with or without them. The cost is storage: `theta` for tens of thousands of cells and thousands of draws makes the trace large. In that case we can keep only the aggregates, or leave the deterministics out and call `pm.compute_deterministics` afterwards. The gain from keeping them in the model is that the estimand lives in the model object, next to the assumptions that justify it, and `az.summary(idata, var_names=["theta_ps_state"])` just works.
#
# ### Prior Predictive Check
#
# Before we look at the data we check what the priors say about the estimand. The prior for $\theta^{\text{PS}}$ spreads over most of the unit interval, which is what we want from weakly informative priors on the logit scale.

# %%
with mrp_model:
    prior_predictive = pm.sample_prior_predictive(draws=2_000, random_seed=rng)

pc = az.plot_dist(prior_predictive, group="prior", var_names=["theta_ps"], figure_kwargs={"figsize": (10, 5)})
pc.viz["figure"].item().suptitle("Prior distribution of the poststratified share", fontsize=18, fontweight="bold");
...
# %%
pc = az.plot_dist(idata, var_names=["theta_ps"], figure_kwargs={"figsize": (10, 5)})
az.add_lines(pc, values={"theta_ps": theta_ps_true}, visuals={"ref_line": {"color": "C2", "label": "truth"}})
az.add_lines(pc, values={"theta_ps": raw_mean}, visuals={"ref_line": {"color": "C3", "linestyle": "--", "label": "raw sample mean"}})
pc.viz["dist"]["theta_ps"].item().set_label("MRP posterior")
ax = pc.get_target("theta_ps", {})
ax.legend(loc="upper left")
pc.viz["figure"].item().suptitle("MRP recovers the population share, the raw mean does not", fontsize=18, fontweight="bold");

# %%
hdi = az.hdi(idata, var_names=["theta_ps"], prob=HDI_PROB)["theta_ps"]

print(f"raw sample mean : {raw_mean:.4f}")
print(f"MRP estimate    : {idata['posterior']['theta_ps'].mean().item():.4f}")
print(f"{HDI_PROB:.0%} HDI         : [{hdi.sel(ci_bound='lower').item():.4f}, {hdi.sel(ci_bound='upper').item():.4f}]")
print(f"true value      : {theta_ps_true:.4f}")

# %% [markdown]
# ### State-Level Estimates
#
# This is where MRP earns its reputation. Some states have a handful of respondents, but their estimates borrow strength from the national pattern and from the state-level predictor $v_s$. We compare the posterior of $\theta^{\text{PS}}_s$ with the truth and with the raw mean of the respondents of each state. The labels show the number of respondents per state.

# %%
n_state = n_cell.sum(axis=(0, 1))
y_state = y_cell.sum(axis=(0, 1))
raw_mean_state = np.divide(y_state, n_state, out=np.full(n_state.shape, np.nan), where=n_state > 0)

state_labels = [f"{state} (n={n})" for state, n in zip(states, n_state, strict=True)]


posterior_shape = (idata["posterior"].sizes["chain"], idata["posterior"].sizes["draw"], len(states))


def as_state_datatree(values: np.ndarray) -> xr.DataTree:
    """Wrap state-level values as a posterior group with dimensions (chain, draw, state)."""
    return az.from_dict(
        {"posterior": {"theta_ps_state": np.broadcast_to(values, posterior_shape)}},
        dims={"theta_ps_state": ["state"]},
...
pc = az.plot_forest(comparison, var_names=["theta_ps_state"], combined=True, figure_kwargs={"figsize": (10, 7)})
pc.add_legend("model")
pc.viz["figure"].item().suptitle("State-level estimates", fontsize=18, fontweight="bold");
...
state_summary = pl.DataFrame(
    {
        "state": states,
        "n": n_state,
        "mrp": idata["posterior"]["theta_ps_state"].mean(dim=("chain", "draw")).to_numpy(),
        "truth": theta_ps_state_true,
        "raw": raw_mean_state,
    }
).with_columns(
    mrp_error=(pl.col("mrp") - pl.col("truth")).abs(),
    raw_error=(pl.col("raw") - pl.col("truth")).abs(),
)

print(f"mean absolute error  MRP: {state_summary['mrp_error'].mean():.4f}   raw: {state_summary['raw_error'].mean():.4f}")
...
print(f"original population : {idata['posterior']['theta_ps'].mean().item():.4f}  (truth {theta_ps_true:.4f})")
print(f"older population    : {posterior_target['theta_ps'].mean().item():.4f}  (truth {theta_ps_target_true:.4f})")
...
generative_model_bad = pm.do(
    ...
    {**true_values, "N": N_cell.astype(float), "v": v_state, "b_state": world["b_state"].to_numpy()},
)
...
az.add_lines(pc, values={"theta_ps": theta_ps_true}, visuals={"ref_line": {"color": "C2", "label": "truth"}})
pc.add_legend("model")
pc.viz["figure"].item().suptitle("Same model, clean diagnostics, wrong answer", fontsize=18, fontweight="bold");
...
#
# On the implementation side, `pymc.dims` made the model read like the math. The effects broadcast to the grid by name, the poststratified quantities are one-line reductions, and the census counts are data we can swap without refitting. The failure case is the part to remember. When the response depends on the answer, the model looks healthy and the estimate is wrong. Identification is an assumption about the world, and the sampler cannot check it for us.

# %% [markdown]
# ## References
#
# - Gelman, A. and Little, T. C. (1997). [Poststratification into many categories using hierarchical logistic regression](https://sites.stat.columbia.edu/gelman/research/published/poststrat3.pdf). *Survey Methodology*, 23(2), 127-135.
# - Park, D. K., Gelman, A. and Bafumi, J. (2004). [Bayesian multilevel estimation with poststratification: State-level estimates from national polls](https://sites.stat.columbia.edu/gelman/research/published/parkgelmanbafumi.pdf). *Political Analysis*, 12(4), 375-385.
...
# - Kennedy, L. and Gelman, A. (2021). [Know your population and know your model: Using model-based regression and poststratification to generalize findings beyond the observed sample](https://doi.org/10.1037/met0000362). *Psychological Methods*, 26(5), 547-558.
...
# - Si, Y., Trangucci, R., Gabry, J. S. and Gelman, A. (2020). [Bayesian hierarchical weighting adjustment and survey inference](https://www150.statcan.gc.ca/n1/pub/12-001-x/2020002/article/00003-eng.htm). *Survey Methodology*, 46(2), 181-214.
...
# - Bareinboim, E. and Pearl, J. (2016). [Causal inference and the data-fusion problem](https://doi.org/10.1073/pnas.1510507113). *Proceedings of the National Academy of Sciences*, 113(27), 7345-7352.
