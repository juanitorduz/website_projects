...
Furthermore, because the model is formulated as a probabilistic graphical model, it naturally supports causal
interventions via Pearl's $\text{do}$-operator \cite{pearl2009causality}, enabling counterfactual analyses that
quantify the revenue impact of hypothetical retention scenarios---a capability we demonstrate on a real-world
dataset in Section~\ref{sec:kt_application}.
...
validates the approach on real-world H\&M transaction data, followed by a discussion of model limitations and trade-offs in Subsection \ref{sec:limitations}. Section \ref{sec:kt_application} provides a second real-world validation using German grocery transaction data with weekly granularity. Finally, Section \ref{sec:conclusion} concludes.
...
\subsubsection{Numerical Evidence: Uncertainty Propagation}

To demonstrate the importance of the coupling mechanism for uncertainty quantification, we compare two model variants:
a \emph{non-coupled} model that uses observed active user counts directly, and the \emph{coupled} model that propagates 
uncertainty through the estimated $N_{\text{active}}$ from the retention component.

\begin{figure}
    \centering
    \includegraphics[width=\textwidth]{images/no_coupling.png}
    \caption{Revenue 94\% HDI for the non-coupled model using observed active user counts. Younger cohorts (bottom panels) 
        with smaller sample sizes show HDIs that are too narrow and fail to capture observed values near zero.}
...
\begin{figure}
    ...
    \caption{Revenue 94\% HDI for the coupled model with uncertainty propagation through $N_{\text{active}}$. 
        The wider HDIs provide better coverage across all cohorts, demonstrating proper uncertainty propagation.}
    \label{fig:coupling}
\end{figure}

Figures \ref{fig:no_coupling} and \ref{fig:coupling} reveal a critical difference in uncertainty quantification. The non-coupled model 
(Figure \ref{fig:no_coupling}) produces HDIs that are systematically too narrow, particularly for younger cohorts where sample sizes are smaller. 
When revenue values approach zero, the non-coupled model's intervals frequently fail to contain the observed values, 
indicating poor calibration. In contrast, the coupled model (Figure \ref{fig:coupling}) produces wider, more realistic HDIs that maintain 
proper coverage across all cohorts. This demonstrates that the coupling mechanism not only provides a principled 
connection between retention and revenue but also ensures that uncertainty in retention estimates appropriately 
propagates to revenue predictions.
...
\begin{itemize}
    ...
    \item \textbf{Counterfactual analysis}: Because the model is specified as a directed graphical model with
          latent retention, one can apply Pearl's $\text{do}$-operator \cite{pearl2009causality} to perform
          interventions---for example, fixing retention to a target level and computing the resulting revenue
          impact. This is fundamentally different from simply changing an input feature in a standard machine
          learning model, as the retention variable $p$ is latent and its downstream effects propagate through
          the coupling mechanism. We illustrate this capability on a real-world grocery dataset in
          Section~\ref{sec:kt_application}.
\end{itemize}
...
a single market or multiple markets with a hierarchical structure.
...
\section{Real-Dataset Application: The H\&M Transactions Dataset}\label{sec:hm_application}
...
in MAPE, where the Bayesian model achieves $10.03\%$ compared to $17.78\%$--$28.07\%$ for the baselines, reflecting
the model's ability to maintain proportionally accurate predictions across cohorts of varying sizes and revenue
magnitudes. The MAE and RMSE metrics tell a similar story: the Bayesian model's MAE of $82.46$ is roughly half
that of the best-performing baseline (SES at $142.64$), demonstrating substantial improvements in absolute
prediction accuracy.
...
\begin{figure}[H]
    ...
    \caption{Total revenue predictions (aggregated across all training cohorts) comparing the Bayesian model with
        baselines. The black line shows observed total revenue, while colored lines show model predictions.}
...
To provide a comprehensive evaluation of our proposed framework, we compare it against the classical buy-till-you-die (BTYD)
...
aggregation reduces noise and improves computational efficiency, it also means that specific customer-level behaviors 
such as individual product preferences or idiosyncratic purchase triggers are not explicitly captured. For applications 
requiring highly personalized interventions, this framework should be seen as a complement to, rather than a 
replacement for, individual-level models.
...
\section{Real-Dataset Application: Grocery Transactions Dataset}\label{sec:kt_application}

To further validate our framework's generalizability across different retail contexts, we apply it to a grocery
transactions dataset from Germany\footnote{The complete implementation code is available at 
\url{https://github.com/juanitorduz/website_projects/blob/master/Python/cohort-revenue-retention/kt_weekly.ipynb}.}.
While the H\&M dataset (Section \ref{sec:hm_application}) represents fashion retail with monthly purchasing cycles,
grocery retail exhibits distinct characteristics: higher purchase frequency, different seasonality patterns driven
by holidays rather than fashion seasons, and typically smaller basket sizes with more regular replenishment behavior.
This application demonstrates the framework's flexibility in adapting to different temporal granularities and
seasonality structures.

\subsection{Dataset and Analysis Period}

The dataset contains transaction records from a German grocery retailer. For our analysis, we focus on the period
from July $2015$ to December $2016$, representing approximately $18$ months of data. Unlike the H\&M analysis
which used monthly cohorts, we aggregate transactions at the \emph{weekly} level, creating weekly cohorts based
on each customer's first purchase week. This finer temporal granularity is appropriate for grocery retail where
purchase cycles are shorter than in fashion retail. The train/test split is set at the end of August $2016$,
holding out approximately $4$ months of data for out-of-sample evaluation.

\subsection{Modeling Seasonality with Gaussian Bump Features}

A key methodological contribution of this application is the use of Gaussian bump features to capture holiday-driven
seasonality, as an alternative to the categorical month encoding used in the synthetic and H\&M examples. Grocery
retail revenue exhibits sharp spikes around specific holidays rather than gradual monthly patterns. We model three
major holiday effects:

\begin{itemize}
    \item \textbf{Black Friday}: A Gaussian bump centered on late November (November $27$, $2015$ and November $25$, $2016$)
          with a narrow bandwidth ($\sigma = 5$ days) to capture the concentrated shopping period.
    \item \textbf{Christmas}: A broader Gaussian bump centered on December $21$ ($\sigma = 7$ days) reflecting the
          extended holiday shopping season.
    \item \textbf{Easter}: A wider Gaussian bump ($\sigma = 14$ days) centered on the variable Easter date
          (April $5$, $2015$ and March $27$, $2016$), accounting for the more diffuse purchasing pattern around this holiday.
\end{itemize}

The Gaussian bump for a holiday date $d_0$ with bandwidth $\sigma$ is defined as:
\begin{align*}
    g(t; d_0, \sigma) = \exp\left(-\frac{(t - d_0)^2}{\sigma^2}\right)
\end{align*}
where $t$ is the observation date. This functional form provides smooth, localized features that capture the
intensity of holiday effects without the discontinuities of indicator variables. Figure \ref{fig:kt_revenue_per_week}
shows the weekly revenue time series with the analysis period highlighted and the three Gaussian bump features overlaid,
illustrating how these features align with observed revenue peaks.

\begin{figure}[H]
    \centering
    \includegraphics[width=\textwidth]{images/kt_revenue_per_week.png}
    \caption{Weekly revenue for the grocery dataset. The shaded blue region indicates the analysis period
        (July $2015$ to December $2016$). The colored curves show the Gaussian bump features for Christmas (blue),
        Easter (orange), and Black Friday (green), which capture holiday-driven seasonality. Note how the
        revenue peaks around Christmas $2015$ and the end-of-year $2016$ period align with the bump features.}
    \label{fig:kt_revenue_per_week}
\end{figure}

\subsection{Model Specification}

We apply the same coupled retention-revenue framework from Section \ref{sec:model_framework}, with the retention
component modeled using BART and the revenue component using a linear model. The key modifications for this
application are:

\begin{enumerate}
    \item \textbf{Weekly granularity}: Features are computed at the weekly level (cohort age in days, age in days)
          rather than monthly.
    \item \textbf{Gaussian bump features}: The BART model for retention includes the three holiday bump features
          alongside cohort age, age, and month indicators. Similarly, the revenue rate $\lambda$ includes
          coefficients for each bump feature.
    \item \textbf{Larger BART ensemble}: We use $m = 200$ trees to capture the more complex weekly patterns.
    \item \textbf{Seasonality effects}: We add seasonality effects to the revenue component.
\end{enumerate}

\subsection{Results}

Figure \ref{fig:kt_revenue_predictions} presents out-of-sample revenue predictions for six selected cohorts spanning
different periods of the analysis window. Several key observations emerge: % \NeedsTeXFormat{LaTeX2e}

\begin{itemize}
    \item \textbf{Well-calibrated uncertainty}: The $94\%$ HDI bands (orange shaded regions) consistently contain
          the observed test values (black lines), demonstrating proper uncertainty quantification even at the
          weekly granularity where variance is higher than monthly aggregates.
    \item \textbf{Decay pattern captured}: The model successfully captures the characteristic revenue decay as
          cohorts age, with older cohorts (e.g., $2015$-$07$-$06$) showing lower absolute revenue levels than
          younger cohorts.
    \item \textbf{Seasonal effects}: The predictions show appropriate seasonal variation in the test period,
          particularly the uptick toward the end of $2016$ corresponding to the holiday season.
    \item \textbf{Information transfer}: Even for the youngest cohort shown ($2016$-$08$-$22$), which has only
          a few weeks of training data, the model produces reasonable predictions by leveraging patterns learned
          from older cohorts.
\end{itemize}

\begin{figure}[H]
    \centering
    \includegraphics[width=\textwidth]{images/kt_revenue_predictions.png}
    \caption{Revenue predictions for six selected cohorts from the grocery dataset.}
    \label{fig:kt_revenue_predictions}
\end{figure}

\subsection{Counterfactual Analysis}

A key advantage of formulating our cohort-revenue-retention model as a probabilistic graphical model is the
ability to perform \emph{counterfactual analysis} using Pearl's $\text{do}$-operator \cite{pearl2009causality}.
The $\text{do}(X = x)$ operator, central to Pearl's causal inference formalism, performs an \emph{intervention}
on the model graph: it fixes a variable $X$ to a specific value $x$ and severs the influence of its parents
in the directed acyclic graph. In the language of graphical models, this corresponds to \emph{graph surgery},
where the incoming edges to the intervened node are removed and the node is replaced by a constant. This
operator is available in both PyMC \cite{pymc2023} and NumPyro \cite{phan2019composable}, making it directly
applicable to models built with either framework.

A crucial observation is that the retention probability $p$ in our model is a \emph{latent} variable, it is
not directly observed but rather inferred from the data through the Binomial likelihood conditioning on the
number of active users $N_{\text{active}}$. This is fundamentally different from a standard machine learning
setting where one might simply change the value of an observed input feature (e.g., modifying a column in a
feature matrix for a gradient boosting model). In our framework, the $\text{do}$-operator intervenes directly
on the model graph, replacing the functional relationship $p = f(\text{cohort age}, \text{age}, \text{month})$
learned by the BART or neural network component with a fixed constant value. The downstream variables, the
number of active users $N_{\text{active}}$ and, through the coupling mechanism, revenue are then recomputed
under this intervention. This is a principled causal operation that would not be possible with a conventional
predictive model where the relationship between retention and revenue is not explicitly encoded in a generative
graphical structure.

To illustrate this capability, we perform an intervention on the grocery dataset by setting the retention
probability to a constant value $\text{do}(p = 0.15)$ for a specific cohort (2015-12-28). This corresponds to
the counterfactual question: ``What would the revenue have been if retention had remained constant at $15\%$?''
The intervened model propagates this fixed retention through the coupling mechanism: the number of active users
$N_{\text{active}} \sim \text{Binomial}(N_{\text{total}}, 0.15)$ is resampled under the intervention, and
the revenue $R \sim \text{Gamma}(N_{\text{active}}, \lambda)$ is computed accordingly, with the rate parameter
$\lambda$ remaining unchanged as it depends on cohort characteristics rather than retention.
Figure~\ref{fig:kt_retention_intervention} shows the results.

\begin{figure}[H]
    \centering
    \includegraphics[width=\textwidth]{images/kt_retention_intervention.png}
    \caption{Counterfactual analysis for cohort 2015-12-28 in the grocery dataset using $\text{do}(p = 0.15)$.
        The top panel shows the retention $94\%$ HDI for the training period (blue) and the intervened constant
        retention value of $0.15$ (magenta horizontal line) applied during the test period. The bottom panel
        shows the resulting counterfactual revenue $94\%$ HDI (orange) compared to the observed revenue (black line)
        in the test period. The posterior predictive mean (orange dots) reflects the revenue that would be expected
        under the hypothetical constant retention scenario. The dashed vertical line indicates the train/test split.}
    \label{fig:kt_retention_intervention}
\end{figure}

This type of counterfactual analysis is essential for strategic and financial decision-making. It enables
business stakeholders to answer ``what-if'' questions about retention scenarios and their downstream revenue
impact. For example, one can evaluate the expected revenue effect of achieving a target retention level through
a marketing campaign, or quantify the financial cost of a decline in retention due to competitive pressure.
These insights directly support resource allocation decisions, retention program evaluation, and financial
planning. Importantly, because the analysis is grounded in a causal intervention on the model graph rather
than a naive input perturbation, the resulting estimates are structurally consistent with the generative
process and account for uncertainty propagation through the coupling mechanism.

\subsection{Discussion}

This grocery retail application demonstrates several important aspects of our framework's flexibility:

\begin{itemize}
    \item \textbf{Temporal granularity}: The framework adapts seamlessly from monthly (H\&M) to weekly aggregation
          without structural changes, simply by adjusting the feature definitions.
    \item \textbf{Alternative seasonality encoding}: Gaussian bump features provide a principled alternative to
          categorical month indicators when seasonality is driven by specific events rather than calendar months.
          This approach is particularly valuable when holidays fall on different dates across years (e.g., Easter)
          or when the effect is concentrated in a narrow time window (e.g., Black Friday).
    \item \textbf{Domain transferability}: The coupled retention-revenue mechanism proves effective across different
          retail contexts, from fashion (lower frequency, higher basket) to grocery (higher frequency, lower basket),
          suggesting broad applicability of the framework.
\end{itemize}

The success on this grocery dataset, combined with the H\&M results, provides evidence that our Bayesian
cohort-revenue-retention framework generalizes well across different retail domains, temporal granularities,
and seasonality structures.

\section{Real-Dataset Application: KOD Dataset}\label{sec:kod_application}

As a complementary evaluation, we apply our framework to the KOD dataset, a non-seasonal retail dataset.
\textbf{[TBA: Detailed dataset description.]}
Unlike the H\&M fashion retail dataset (Section \ref{sec:hm_application}) and the German grocery dataset
(Section \ref{sec:kt_application}), which exhibit pronounced seasonal patterns, the KOD dataset lacks strong
seasonality. This makes it a particularly interesting testbed: in the absence of seasonal effects, the stationarity
assumption underlying classical BTYD models (BG/NBD + Gamma-Gamma) is well-suited to the data, and we expect
both approaches to perform comparably.

We benchmark our Bayesian cohort-revenue-retention model (with BART for the retention component) against the same
set of baselines used in previous sections, as well as the BG/NBD + Gamma-Gamma model implemented in
PyMC-Marketing \cite{pymc_marketing}. Figure \ref{fig:kod_cohort_predictions} shows the cohort-level revenue
predictions from the Bayesian model alongside the benchmark approaches, and Figure \ref{fig:kod_btyd} presents
the corresponding predictions from the BTYD model.

\begin{figure}[H]
    \centering
    \includegraphics[width=\textwidth]{images/kod_cohort_predictions.png}
    \caption{Revenue predictions comparing the Bayesian cohort-revenue-retention model (with $94\%$ HDI) against
        benchmark approaches for selected cohorts of the KOD dataset.}
    \label{fig:kod_cohort_predictions}
\end{figure}

\begin{figure}[H]
    \centering
    \includegraphics[width=0.8\textwidth]{images/kod_btyd.png}
    \caption{Revenue predictions using the BG/NBD + Gamma-Gamma model (PyMC-Marketing) for selected cohorts
        of the KOD dataset.}
    \label{fig:kod_btyd}
\end{figure}

Table \ref{tab:kod_metrics} presents the out-of-sample evaluation metrics for all models.

\begin{table}[H]
    \centering
    \caption{Out-of-sample evaluation metrics for revenue prediction on the KOD dataset. Lower values indicate
        better predictive performance.}
    \label{tab:kod_metrics}
    \begin{tabular}{lccc}
        \hline
        \textbf{Model}                        & \textbf{MAE} & \textbf{RMSE} & \textbf{MAPE (\%)} \\
        \hline
        Bayesian (BART)                       & 453.89       & 570.30        & 20.19              \\
        AutoMFLES                             & 1731.88      & 2054.91       & 73.42              \\
        SES                                   & 1320.75      & 1861.45       & 54.27              \\
        HistGradientBoostingRegressor         & 745.79       & 864.66        & 35.75              \\
        BG/NBD + Gamma-Gamma                  & 514.61       & 830.05        & 19.28              \\
        \hline
    \end{tabular}
\end{table}

The results confirm our expectation: the Bayesian cohort model and the BTYD model achieve comparable MAPE
($20.19\%$ vs.\ $19.28\%$), with both substantially outperforming the time series and machine learning baselines.
The Bayesian model achieves a lower MAE ($453.89$ vs.\ $514.61$) and RMSE ($570.30$ vs.\ $830.05$), while
the BTYD model edges ahead in MAPE. This near-parity is consistent with the non-seasonal nature of the data:
without calendar-driven patterns to capture, the stationarity assumption of the BG/NBD model is appropriate,
and the cohort framework's ability to model seasonal effects does not confer an additional advantage.

\begin{remark}
This result complements the grocery dataset application (Section \ref{sec:kt_application}), where the
cohort-level framework substantially outperforms BTYD models due to the strong seasonality component in
grocery retail. The KOD dataset, lacking such seasonality, represents a setting where the stationarity
assumption of the BG/NBD model is appropriate, and accordingly the two approaches yield comparable
performance. Together, these two applications illustrate the framework's versatility: it matches BTYD
models in their favorable regime (non-seasonal data) while significantly outperforming them when
...
allowing for the incorporation of rich covariate information when needed. The validations on the H\&M fashion retail dataset 
(Section \ref{sec:hm_application}), the German grocery dataset (Section \ref{sec:kt_application}), and the KOD dataset 
(Section \ref{sec:kod_application}) demonstrate that this cohort-level framework delivers superior predictive performance 
compared to standard time series and machine learning baselines, generalizing across different retail domains, temporal 
granularities (monthly vs.\ weekly), and seasonality structures. Moreover, the KOD application shows that in non-seasonal 
settings the framework performs comparably to classical BTYD models, while the grocery application confirms its clear 
advantage when seasonality is present. The coupling mechanism provides crucial information transfer, particularly for 
cohorts with limited training history.

Table \ref{tab:unified_dataset_metrics} provides a unified out-of-sample comparison of all evaluated datasets and benchmark models,
highlighting robust Bayesian performance across seasonal and non-seasonal settings.
Here, SES (Simple Exponential Smoothing) is a classical level-based time-series baseline, AutoMFLES is an automated
multiple-seasonality forecasting model, and HistGradientBoostingRegressor is a tree-based gradient boosting machine-learning baseline.

\begin{table}[H]
    \centering
\caption{Unified out-of-sample evaluation metrics for revenue prediction across the H\&M, Grocery, and KOD datasets.
        Lower values indicate better predictive performance. Across these three datasets, the Bayesian BART model
        dominates overall, with only one near-exception where BG/NBD + Gamma-Gamma is slightly better on KOD MAPE.
        This is consistent with KOD being relatively uniform, with weak seasonality and no additional covariates,
        a setting where classical BTYD models typically perform well.}
    \label{tab:unified_dataset_metrics}
    \begin{tabular}{llccc}
        \hline
        \textbf{Dataset} & \textbf{Model}                & \textbf{MAE} & \textbf{RMSE} & \textbf{MAPE (\%)} \\
        \hline
        H\&M             & \textbf{Bayesian (BART)}      & \textbf{82.46}        & \textbf{135.64}        & \textbf{10.03}              \\
        H\&M             & AutoMFLES                     & 190.39       & 256.97        & 28.07              \\
        H\&M             & SES                           & 142.64       & 193.19        & 17.78              \\
        H\&M             & HistGradientBoostingRegressor & 161.90       & 218.32        & 25.12              \\
        H\&M             & BG/NBD + Gamma-Gamma          & 405.77       & 632.81        & 65.90              \\
        \hline
        Grocery          & \textbf{Bayesian (BART)}      & \textbf{636.24}       & \textbf{831.40}        & \textbf{6.97}               \\
        Grocery          & AutoMFLES                     & 3705.04      & 4317.04       & 38.99              \\
        Grocery          & SES                           & 1055.49      & 1352.63       & 12.86              \\
        Grocery          & HistGradientBoostingRegressor & 944.63       & 1174.86       & 10.77              \\
        Grocery          & BG/NBD + Gamma-Gamma          & 1253.55  	& 3100.92	    & 8.75              \\
        \hline
        KOD              & \textbf{Bayesian (BART)}      & \textbf{453.89}       & \textbf{570.30}        & 20.19              \\
        KOD              & AutoMFLES                     & 1731.88      & 2054.91       & 73.42              \\
        KOD              & SES                           & 1320.75      & 1861.45       & 54.27              \\
        KOD              & HistGradientBoostingRegressor & 745.79       & 864.66        & 35.75              \\
        KOD              & BG/NBD + Gamma-Gamma          & 514.61       & 830.05        & \textbf{19.28}              \\
        \hline
    \end{tabular}
\end{table}
