...
applied to real-world business datasets (both in contractual and non-contractual settings). As we will describe below, the
...
(and actually, the time-varying covariates require significant amoutn of work to implement in a way that scales).
...
\subsubsection*{Age-Period-Cohort Models.} The statistical literature on age-period-cohort (APC) modeling provides a rich
...
\subsubsection*{Survival Analysis and Retention Modeling.} Recently, \cite{hubbard2021beta} introduced Beta Survival Models
...
\subsubsection*{Customer Lifetime Value Models.} The customer lifetime value literature, pioneered by the work of Fader and
...
applications where the seasonal componets and additional covariates are important, for the mid-term decision making, our approach
tends to be more accurate and flexible.

\subsection*{The Cohort-Revenue-Retention Modeling Strategy}
...
shows an example of a cohort-level retention matrix. These cohorts are tipically defined by business rules, e.g.
customers who registered, donwloaded the app or did their first purchase during the same month.
Here we encode the cohort retention as a function of time (period). Note that we exclude
the diagonal as it is uninformative (always containing ones). Observe that older cohorts have more data (obviously), so
we would like to use this information to improve the estimation of retention for younger cohorts. That is, we do not want
to model each cohort independently but rather the {\em whole retention matrix} (we will do the same for the revenue
matrix and couple them together). As in most customer lifetime value models, we do not only want to model the retention matrix,
but also the corresponding revenue matrix, shown in Figure \ref{fig:revenue_matrix}. As in the retention case, we want to
make sure we use all the information available to improve the estimation of revenue for younger cohorts. How can we do this?
We start with a non-parametric approach to model the retention matrix. Namely, we use Bayesian additive regression trees
(implementation in PyMC following \cite{quiroga2022bart}) to model the retention matrix, capturing the probability that a
customer from a given cohort remains active in subsequent periods. Then, we couple this model with an additional linear model
to model the revenue matrix, predicting how much revenue active customers will generate. This dual approach balances the
flexibility needed to capture complex retention patterns with the interpretability desired for revenue forecasting. That being said,
this framework is flexible enough to incorporate additional parametrizations for the revenue component. The true key ingredients are:
having a Bayesian model with two likelihoods (retention and revenue) that are coupled through the number of active users and
the ability to add additional covariates to the model.

An important strength of this modular framework is its extensibility to more complex structures. Namely, we also present
...
\subsection*{Cohort Age and Period Features}

Before going into the details of the model specification, we need to define the core features that are be used in the model.
...
As mentioned above, the main idea behind the specification is to model each revenue and retention matrices, using the core
features above, and couple them together. Specifically, we have the following components:
...
\begin{itemize}
    \item {\bf Retention Component}: We model the number of active users $N_{\text{active}}$ in each cohort as a binomial random
          ...
          using a BART model with features cohort age, age, and month. This last feature is crucial to capture the seasonal patterns.
          This flexible approach allows the model to capture non-linear relationships and interactions between features.

    \item {\bf Revenue Component}: We model the revenue matrix through a gamma random variable
          $\text{Gamma}(N_{\text{active}}, \lambda)$, as we want to ensure non-negative values (Gamma distributions have been used
          widely to model monetary components, e.g. the Gamma-Gamma model from \cite{fader2005rfm}).
          ...
          However, seasonal features could be added if needed (plus additional features and different parametrizations,
          for example multiplicative effects).

    \item {\bf Coupling Mechanism}: The retention and revenue coupling is a key feature of this work. We couple the two components
          through the number of active users, extending decomposition ideas similar to those in the Gamma-Gamma model,
          which separates transaction frequency from monetary value. The full model specification is as follows:
...
Our goal is to simultaneously estimate the BART parameters and the beta coefficients (including the intercept) of
the linear component. We want to do this for two reasons. First,to understand the contribution of each feature to the retention
and revenue over time. Secondly, use the model and inferred parameters to generate out-of-sample predictions of the retention
and revenue matrices. This is extremely important for scenario and business planning. Typical applications are to use these
forecasted values to understand the true value of the customer base, financial planning or to generate {\em counterfactuals} for
market-level interventions where we expect different cohorts to react differently. \\

This modeling strategy offers several distinct advantages:

\begin{itemize}
    \item \textbf{Flexibility in relationship modeling}: By employing Bayesian additive regression trees (BART), our approach
          can capture complex non-linear relationships between cohorts, time periods, and behavioral metrics without requiring
          explicit specification of these relationships. We have also experimented with Bayesian neural networks to model the
          retention and revenue matrices, and the results were similar.
...
\section{A Synthetic Dataset}\label{sec:synthetic_dataset}
...
business scenarios, incorporating the types of temporal patterns and cohort dynamics commonly observed.
...
motivates our modeling choices and provides a baseline for evaluating model performance. Figure \ref{fig:retention_matrix}
...
\subsubsection{Retention Component}
...
When we run the inference process (MCMC or SVI), we condition on the number of active users $N_{\text{active}}$ and not
directly on the observed retention rates. The reason to treat the retention as a latent variable is to have better uncertainty
estimates for it depending on the cohort size ($N_{\text{total}}$) and the number of active users ($N_{\text{active}}$). Recall
that the standar error of a proportion is $\sqrt{p(1-p)/N_{\text{total}}}$.\\
...
\begin{remark}[Baseline Model]
    ...
    iteration to get the decision-making process moving forward. Nevertheles, in many real applications, we have seen that
...
model seasonality in the revenue component, as seasonal patterns are already captured by the retention component. Still,
if the business model ad hand also has a strong seasonal component in average basket sizes, then an additional seasonal component
can be added to the revenue model.

\begin{remark}[Chort Age Encoding]
...
we follow the {\em Bayesian workflow} (\cite{gelman2020bayesianworkflow}). After fitting the model using MCMC with the
computational settings described above, we conduct thorough diagnostics to ensure reliable inference. Figure \ref{fig:posterior_predictive}
presents a critical diagnostic: the posterior predictive distribution for both model
...
provides an adequate fit to the data. Beyond the posterior predictive check, we examine convergence diagnostics for
...
The data generation process maintains the same retention and revenue dynamics described in Section \ref{sec:synthetic_dataset}, but now each market has
...
\begin{itemize}
    ...
    \item \textbf{Appropriate uncertainty quantification}: The highest density intervals are wider for Market $C$ than for more
          established markets (e.g. Market $A$, for which the results look almost the same as in Section \ref{sec:model_specification}),
...
\section{A Real-Dataset Application}

Having demonstrated our framework on synthetic data with known ground truth, we now validate its practical applicability
using a large-scale real-world dataset. This application addresses a key requirement for any proposed methodology:
demonstrating performance on actual business data where ground truth patterns are unknown and must be discovered.

\subsection{The H\&M Transactions Dataset}
...
on their first purchase date, resulting in $18$ cohorts from November $2018$ to April $2020$. We use a train/test split
...
\begin{itemize}
    ...
    \item Older cohorts (e.g., November--December $2018$) exhibit higher retention rates, likely reflecting a
          selection effect where customers acquired earlier have demonstrated longer-term engagement.
...
\begin{figure}[H]
    ...
    \caption{Retention matrix for the H\&M dataset (training period). Each cell shows the proportion of customers
        from a given cohort (row) who made at least one purchase in each subsequent period (column). The characteristic
        decay pattern with cohort age is evident, along with some seasonal variation.}
...
\begin{figure}[H]
    ...
    \caption{Revenue matrix for the H\&M dataset (training period). Revenue values are shown for each cohort-period
        combination, with color intensity indicating magnitude. The strong correlation with active user counts
        (reflected in the retention matrix) supports our coupled modeling approach.}
    \label{fig:hm_revenue_matrix}
\end{figure}

\subsection{Model Comparison with Baseline Approaches}

To address the reviewer's request for comparison with existing frameworks, we benchmark our Bayesian cohort-revenue-retention
model against three established approaches commonly used for time series forecasting in business contexts:
...
These baselines represent the spectrum of approaches typically considered for cohort-level forecasting: classical
statistical methods (SES), automated time series decomposition (AutoMFLES), and machine learning (gradient boosting).
Importantly, these methods treat each cohort's time series independently, without the information sharing across
cohorts that our framework provides.

Figure \ref{fig:hm_retention_predictions} shows retention predictions for selected cohorts, demonstrating that our
model successfully captures the retention dynamics with well-calibrated $94\%$ highest density intervals that
contain the observed test values.

\begin{figure}[H]
    \centering
    \includegraphics[width=\textwidth]{images/hm-transactions_58_0.png}
    \caption{Out-of-sample retention predictions for selected cohorts from the H\&M dataset. The blue shaded region
        shows the $94\%$ HDI for training data, while the orange region shows the $94\%$ HDI for test predictions.
        The vertical dashed line indicates the train/test split. Observed retention values (points) fall within
        the predicted intervals, demonstrating well-calibrated uncertainty quantification.}
    \label{fig:hm_retention_predictions}
\end{figure}

Figure \ref{fig:hm_revenue_predictions} presents the key comparison for revenue forecasting. The results reveal
a striking pattern: while the baseline methods (AutoMFLES, SES, and HistGradientBoostingRegressor) provide reasonable
fits to historical data, they fail dramatically in out-of-sample prediction, particularly for newer cohorts with
limited history. In contrast, our Bayesian model produces accurate forecasts with appropriate uncertainty bounds
across all cohorts.

\begin{figure}[H]
    \centering
    \includegraphics[width=\textwidth]{images/hm-transactions_64_0.png}
    \caption{Revenue predictions comparing the Bayesian model against baseline approaches. Each panel shows a different
        cohort, with the Bayesian model's $94\%$ HDI (shaded regions) compared to point forecasts from AutoMFLES (green),
        SES (magenta), and HistGradientBoostingRegressor (brown). The baseline methods fail catastrophically for newer
        cohorts (bottom panels), while our model maintains accurate predictions by borrowing strength across cohorts.}
    \label{fig:hm_revenue_predictions}
\end{figure}

The failure of baseline methods is particularly pronounced for cohorts with limited training history (e.g., the
...
Figure \ref{fig:hm_total_revenue} aggregates predictions across all training cohorts to show total revenue forecasting
...
Table \ref{tab:hm_metrics} presents quantitative evaluation metrics comparing out-of-sample prediction accuracy
...
\begin{table}[H]
    ...
    \caption{Out-of-sample evaluation metrics for revenue prediction on the H\&M dataset. Lower values indicate
        better predictive performance. The Bayesian model substantially outperforms all baseline approaches.}
...
\subsection{Key Insights from the Real-Data Application}

This application to the H\&M dataset yields several important insights:

\begin{enumerate}
    \item \textbf{Information pooling is critical}: The dramatic failure of per-cohort baseline methods underscores
          the importance of sharing information across cohorts. Our framework's ability to learn common patterns
          (retention decay, seasonality, revenue-per-user dynamics) while allowing cohort-specific variation
          enables reliable predictions even for data-sparse cohorts.

    \item \textbf{Coupled modeling improves accuracy}: By jointly modeling retention and revenue through the
          number of active users, our approach captures the fundamental business relationship between customer
          engagement and revenue generation. This coupling provides more stable predictions than modeling
          revenue in isolation.

    \item \textbf{Uncertainty quantification enables better decisions}: The baseline methods provide only point
          forecasts, offering no guidance on prediction reliability. Our Bayesian framework's well-calibrated
          uncertainty intervals allow decision-makers to appropriately weight predictions and plan for a range
          of scenarios.

    \item \textbf{The framework scales to real-world data}: With over a million customers and $18$ cohorts,
          this application demonstrates that our methodology handles realistic data volumes while maintaining
