...
Media-spend time series are typically highly collinear, because campaigns across channels are launched on the same calendar windows; in strongly seasonal businesses such as food delivery, retail flash sales, or travel, paid search, paid social, video, and display are often activated together around the same demand peaks.
This co-activation alone is enough to leave channel-level effects weakly identified even before any latent driver is introduced.
...
Prior elicitation now operates on a quantity that marketers, finance, and product owners can reason about in orders of magnitude.
When several lift experiments are available on the same channel, \citet{wurm2024mmmcalibration} recommend constructing the prior on $\ROAS_k$ as a weighted sum of the per-experiment ROAS priors with weights proportional to the experimental media spend; this collapses every experiment into a single channel-level prior and discards the information that each experiment carried about the saturation curve at the spend level where it was actually run.
The saturation-likelihood calibration of Section~\ref{sec:liftlik} retains that spend-level information by entering each experiment as its own likelihood term anchored at the spend pair where it was measured.
...
\begin{multline}\label{eq:lift-lik}
    |\Delta y| \;\sim\; \mathrm{Gamma}\bigl(\mu_k,\, \sigma\bigr),\\
    \mu_k = \bigl|S_{\lambda_k}\!\bigl(A_{\alpha_k}(x + \Delta x)\bigr) - S_{\lambda_k}\!\bigl(A_{\alpha_k}(x)\bigr)\bigr|,
\end{multline}
using the same adstock and saturation transforms that already appear in (\ref{eq:scm}) and PyMC's $(\mu,\sigma)$ parametrization of the Gamma distribution.
...
Because M2 overstates channel~1's effect, the naive policy over-allocates to the confounded channel; both M3 and M4 reverse this and reproduce the oracle's ranking, with $\hat\pi^{M4}$ tracking the oracle most closely.
% TODO: replace placeholder with the measured percentage from the decision-loss evaluator.
Across a sweep of total budgets, M4 closes \emph{approximately X\%} of the revenue gap $V(\pi^\star) - V(\hat\pi^{M2})$ between the naive and oracle policies and M3 closes a smaller but substantial fraction; the residual gap is attributable to weak identification of the saturation shape.
% TODO: include figures/fig4_decision_loss.pdf
The same expected-revenue objective extends to a risk-aware Bayesian objective that trades off mean response against tail risk in the posterior predictive (Value-at-Risk, or a mean-tightness criterion that penalizes posterior dispersion); tighter and unbiased posteriors on the saturation curves, the output of calibration, translate directly into better risk-adjusted allocations, not only better expected-value ones \citep{pymcmarketing_allocation_assessment}.
Reporting in policy space, rather than coefficient space, aligns the metric with the decision-making premise: estimation error and decision error are not the same thing.

\subsection{Sensitivity to Prior Misspecification}\label{sec:sensitivity}

Lift tests are themselves biased by spillover, market heterogeneity, and staleness, so we probe robustness by sweeping the experimental prior mean across $\{0.5, 0.75, 1.0, 1.25, 1.5\}\times \ROAS_k^\text{true}$ and the scale $\sigma$ over a range, measuring decision loss (\ref{eq:loss}) for each combination.
% TODO: insert sensitivity heatmap (figures/fig5_sensitivity.pdf) once available.
The calibrated MMM is robust to $\pm 25\%$ prior misspecification at moderate $\sigma$ and degrades substantially only when the prior is simultaneously biased and tight; the practical implication is to widen $\sigma$ when experiments are few, stale, or known to be biased, rather than tightening it to chase a sharper ROAS posterior.

\section{Discussion}\label{sec:discussion}

The recipe is a concrete instance of the data-fusion programme of \citet{bareinboim2016causal}, sharing its structural logic with the literatures on combining randomized trials and observational studies in healthcare \citep{colnet2024causal,kallus2018removing,yang2020combining}, off-policy evaluation \citep{dudik2014ope}, and causal bandits \citep{lattimore2016causalbandits}; what is specific to MMM is that the observational data is dense in spend levels but confounded while the experimental data is unconfounded but sparse and local, so the saturation-likelihood term is the natural way to let each experiment locate the curve at the spend pair it measured while the observational data shapes the rest.
The argument is therefore not anchored on the simulation in Section~\ref{sec:experiments}: that simulation supplies a setting where ground truth is available to verify the bias correction, but the underlying problem (multicollinear co-activated channels with unobserved demand drivers) is a routine feature of production MMM deployments, where the same recipe applies whether or not an oracle is available to check against.
The recipe is also not specific to the single-pooled-channel setting used here: the saturation-likelihood term plugs directly into geo-level hierarchical MMMs in the style of \citet{sun2017geolevel}, where lift tests in a subset of geos update population hyperparameters and propagate evidence to unobserved geos through the hierarchy; a worked PyMC-Marketing implementation reports roughly a $40\%$ tightening of saturation posteriors at no cost to predictive fit \citep{pymcmarketing_geolift_calibration}.

The limitations are as follows.
First, the single confounder used in Section~\ref{sec:experiments} is a didactic choice; both the ROAS reparameterization and the saturation-likelihood calibration extend without modification to multiple, non-stationary, and cross-geo dependent confounders.
...
Third, saturation and adstock parameters remain weakly identified even under calibration (in our M3 fit the $94\%$ interval on $\lambda_2$ is $[0.073, 2.986]$ around a true value of $2.5$); the remedy is experimental rather than structural, since additional lift tests at distinct spend levels collapse the ridge documented by \citet{vincent2026liftasymptote}.
...
Future work should connect this calibration scheme to causal bandits, hierarchical pooling across geographies and brands \citep{masoero2025multiple}, and debiasing priors for known-biased experiments.
