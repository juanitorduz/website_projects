...
title(main = str_c("Random Model - ROC AUC (Train) = ", auc_random))
```
...
```{r}
pred_df <- get_pred_df(model_obj = pls_model_1, X = X_test)
# Confusion Matrix. 
...
plot(roc_curve_test)
title(main = str_c("PLS Model 1 - ROC AUC (Test) = ", auc_pls))
```
...
```{r}
pred_df <- get_pred_df(model_obj = gbm_model_1, X = X_test)
# Confusion Matrix. 
conf_matrix_test <- confusionMatrix(data = pred_df$y_pred, reference =  y_test)
conf_matrix_test
```
...
plot(roc_curve_test)
title(main = str_c("GBM Model 1 - ROC AUC (Test) = ", auc_gbm))
```
...
```{r}
pred_df <- get_pred_df(model_obj = pls_model_2, X = X_test)
# Confusion Matrix. 
conf_matrix_test <- confusionMatrix(data = pred_df$y_pred, reference =  y_test)
conf_matrix_test
```
...
```{r}
pred_df <- get_pred_df(model_obj = gbm_model_2, X = X_test)
# Confusion Matrix. 
conf_matrix_test <- confusionMatrix(data = pred_df$y_pred, reference =  y_test)
conf_matrix_test
```
...
plot(roc_curve_eval)
title(main = str_c("PLS Model 3 - ROC AUC (Eval) = ", auc_pls))
```
...
```{r, fig.align="center"}
pred_df <- get_pred_df(model_obj = pls_model_3, X = X_test, threshold = best_point_eval["threshold"])
...
We indeed see that the sensitivity increases to around 0.8 on the test set. The tradeoff is a decrease in specificity.
...
plot(roc_curve_eval)
title(main = str_c("GBM Model 3 - ROC AUC (Eval) = ", auc_gbm))
```
...
```{r, fig.align="center"}
pred_df <- get_pred_df(model_obj = gbm_model_3, X = X_test, threshold = best_point_eval["threshold"])
...
```{r}
pred_df <- get_pred_df(model_obj = pls_model_4, X = X_test)
# Confusion Matrix. 
conf_matrix_test <- confusionMatrix(data = pred_df$y_pred, reference =  y_test)
conf_matrix_test
```
...
plot(roc_curve_test)
title(main = str_c("PLS Model 4 - ROC AUC (Test) = ", auc_pls))
```
...
```{r}
pred_df <- get_pred_df(model_obj = gbm_model_4 , X = X_test)
# Confusion Matrix. 
conf_matrix_test <- confusionMatrix(data = pred_df$y_pred, reference =  y_test)
conf_matrix_test
```
...
plot(roc_curve_test)
title(main = str_c("GBM Model 4 - ROC AUC (Test) = ", auc_gbm))
```
...
```{r}
pred_df <- get_pred_df(model_obj = pls_model_5 , X = X_test)
# Confusion Matrix. 
conf_matrix_test <- confusionMatrix(data = pred_df$y_pred, reference =  y_test)
conf_matrix_test
```
...
plot(roc_curve_test)
title(main = str_c("PLS Model 5 - ROC AUC (Test) = ", auc_pls))
```
...
```{r}
pred_df <- get_pred_df(model_obj = gbm_model_5 , X = X_test)
# Confusion Matrix. 
conf_matrix_test <- confusionMatrix(data = pred_df$y_pred, reference =  y_test)
conf_matrix_test
```
...
plot(roc_curve_test)
title(main = str_c("GBM Model 5 - ROC AUC (Test) = ", auc_gbm))
```
